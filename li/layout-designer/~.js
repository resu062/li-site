...
class BaseItem {
    ...
    get actions() {
        if (!LI._$$[this.$$id]._$$.actions[this.level]) LI._$$[this.$$id]._$$.actions[this.level] = [];
        return LI._$$[this.$$id]._$$.actions[this.level];
    }
    get level() { return this._level || this.$root._level || this.id }

    get checked() { return this._checked; }
    set checked(v) {
        this._checked = v;
        this.applyAction(v ? 'checked' : 'unchecked');
    }
    get expanded() { return this._expanded; }
    set expanded(v) {
        this._expanded = v;
        this.applyAction(v ? 'expanded' : 'unexpanded');
    }
    applyAction(name, v) {
        if (!this.$$.designMode) return;
        const item = { action: name, props: { item: this.id + '' } };
        this.actions.splice(this.actions.length, 0, item);
        saveToLocalStorage(this, true, (this.level2 || this.level || 'main'))
    }
    ...
    get items() {
        if (!this._items) {
            ...
            if (this._items.length) {
                ...
                this._level2 = this.$root.level || 'main';
                this._level = this.id;
                LI._$$[this.$$id]._$$.actions[this.level] = [];
                loadFromLocalStorage(this, this._level);
            }
        }
        return this._items;
    }
...
function loadFromLocalStorage(item, level) {
    let actions = localStorage.getItem(item.fileName);
    actions = actions ? JSON.parse(actions) : {};
    item.$$.actions['' + (level || item.level)] = actions['' + (level || item.level)] || [];
}
function saveToLocalStorage(item, save, level) {
    if (save && item.$$.designMode && item.actions) {
        item.$$.actions['' + (level || item.level)] = item.actions;
...
customElements.define('li-layout-structure', class LiLayoutStructure extends LiElement {
    ...
    updated(changedProps) {
        ...
        if (changedProps.has('layout') && this.layout) {
            if (this.layout.actions) this.layout.actions.forEach(action => this[action.action](this.layout, action.props));
            this.$$update();
        }
    }
    ...
    execute(action, item = this.layout) {
        ...
        this[action.action](this.layout, action.props);
    }
    move(props) {
    ...
    checked = (item, props) => {
        const source = findRecursive.call(item.$root, props.item);
        if (source)
            source.setChecked(true);
    }
    unchecked = (item, props) => {
        const source = findRecursive.call(item.$root, props.item);
        if (source)
